...
Für Realisierungen wird die Capability-Menge nicht um `SETUID` oder `SETGID` erweitert. Der Adapter deaktiviert Nix' Build-User-Umschaltung mit `build-users-group = ""`; Derivations laufen damit als root **innerhalb des Wegwerfcontainers**. Der Realbuild-Container erhält zusätzlich zu `CHOWN` und `DAC_READ_SEARCH` genau `FOWNER` und `DAC_OVERRIDE`: `FOWNER` erlaubt Nix die reproduzierbar nötige Modusänderung an fremdbesessenen `/nix/var/nix/builds/nix-*`-Verzeichnissen; `DAC_OVERRIDE` erlaubt das anschließende reproduzierbar nötige Entfernen fremdbesessener Dateien aus diesen Buildbäumen. Beide Capabilities wurden separat gegen die konkreten Fehlerfälle begründet. Repository-Mount und Git-Metadaten bleiben read-only, der Container-Socket bleibt abwesend, `no-new-privileges` und `--cap-drop=ALL` bleiben aktiv. Die synchrone MCP-v0-Ausführung erhält weder `FOWNER` noch `DAC_OVERRIDE`. Container-root ist keine Host-root-Autorität.
