...
def test_flake_exports_package_nixos_module_and_evidence_adapter() -> None:
    ...
    assert 'id="malformed_barrier"' in source
    assert "for _ in malformed_requests:" in source
    assert "response_message = await write_receive_stream.receive()" in source
    assert "barrier_message = await write_receive_stream.receive()" in source
    assert "with pytest.raises(anyio.WouldBlock):" in source
    assert "write_receive_stream.receive_nowait()" in source
